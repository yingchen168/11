...
const NOW = new Date('2026-03-19T00:47:00Z');
...
function materiallyChanged(oldItem, newItem){
  ...
  if (oldItem.direction !== newItem.direction) return true;
  if (Math.abs((oldItem.probability||0) - newItem.probability) >= 1.0) return true;
  const oa = oldItem.spread ?? oldItem.priceDiff ?? null;
  if (oa != null && Math.abs(oa - newItem.spread) >= 0.002) return true;
  const ol = oldItem.liquidity || 0;
  if (ol > 0 && Math.abs(newItem.liquidity - ol) / ol >= 0.2) return true;
  if ((oldItem.expiry||oldItem.end) !== newItem.expiry) return true;
  return false;
}
...
for(const m of markets){
  try{
    ...
    if(idx < 0) continue;
    if(!(bestP >= 0.80 && bestP <= 0.98)) continue;
